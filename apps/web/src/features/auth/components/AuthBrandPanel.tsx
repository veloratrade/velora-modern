...
import { FeatChartIcon, FeatShieldIcon, FeatCardIcon } from "./authIcons";

type Props = {
  variant: "login" | "register";
  t: (k: string, p?: Record<string, unknown> | null, fb?: string) => string;
  number: (v: unknown, o?: Intl.NumberFormatOptions) => string;
  percent: (v: unknown, o?: Intl.NumberFormatOptions) => string;
};

export function AuthBrandPanel({ variant, t, number, percent }: Props) {
  ...
  return (
    <div className="brand-panel">
      ...
      <h2>
        ...
        <br />
        <span>{t("common.in.8f5d0068", null, "در")}</span> <span className="grad">{t(`${p}.world.class.88072244`, null, "سطح جهانی")}</span>
      </h2>
      <p className="sub">
        {t(`${p}.record.every.trade.and.take.your.strategies.92bc640a`, null, "هر معامله‌تان را ثبت کنید و با شاخص‌های حرفه‌ای، استراتژی‌های خود را به سطح بعدی ببرید.")}
      </p>
      <div className="chart-wrap">
        <div className="chart-head">
          <span className="lbl">{t(`${p}.equity.curve.sample.9799e6be`, null, "منحنی سرمایه — نمونه")}</span>
          <span className="val">
            <span>{percent(0.324, { signDisplay: "always", maximumFractionDigits: 1 })}</span>
            <em>▲</em>
          </span>
        </div>
        <svg className="chart-svg" preserveAspectRatio="none" viewBox="0 0 420 110">
          <defs>
            <linearGradient id="lineGradAuth" x1="0" x2="1" y1="0" y2="0">
              <stop offset="0" stopColor="#b88d1d" />
              <stop offset=".5" stopColor="#fce38a" />
              <stop offset="1" stopColor="#d4af37" />
            </linearGradient>
            <linearGradient id="areaGradAuth" x1="0" x2="0" y1="0" y2="1">
        ...
        <div className="float-chip fc-1">
          ...
          <span>
            <span>{percent(0.24, { signDisplay: "always", maximumFractionDigits: 0 })}</span>
        ...
        <div className="float-chip fc-2">
          ...
          <span>
            R <span>{number(2.1, { minimumFractionDigits: 1, maximumFractionDigits: 1 })}</span>
...
// Register-only brand features variant
export function RegisterBrandFeatures({ t }: { t: Props["t"] }) {
  return (
    <div className="brand-feat">
      <div className="bf">
        ...
        <div>
          <b>{t("pages.register.deep.performance.analysis.78e2ed0b", null, "تحلیل عمیق عملکرد")}</b>
          <span>{t("pages.register.win.rate.profit.factor.average.r.and.271981f1", null, "نرخ برد، فاکتور سود، میانگین R و منحنی سرمایه")}</span>
        </div>
      </div>
      <div className="bf">
        <div className="ic">
        ...
        <div>
          <b>{t("pages.register.bank.level.security.70c4bc21", null, "امنیت در سطح بانکی")}</b>
          <span>{t("pages.register.token.encryption.two.factor.authentication.and.session.2720f3fd", null, "رمزنگاری توکن‌ها، ورود دو مرحله‌ای و مدیریت نشست‌ها")}</span>
        </div>
      </div>
      <div className="bf">
        <div className="ic">
        ...
        <div>
          <b>{t("pages.register.synced.with.mt4.mt5.e94069d4", null, "همگام با MT4 / MT5")}</b>
          <span>{t("pages.register.automatic.trade.logging.and.multi.broker.account.423e2a76", null, "ثبت خودکار معاملات و مدیریت چند حساب بروکر")}</span>
        </div>
      </div>
    </div>
  );
