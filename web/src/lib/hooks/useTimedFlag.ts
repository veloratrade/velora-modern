...
/** A message that shows itself and auto-hides after `ms` (inline error/ok boxes, toasts). */
export function useTimedMessage<T = string>(ms: number) {
  const [value, setValue] = useState<T | null>(null);
  const [visible, setVisible] = useState(false);
  const timer = useRef<ReturnType<typeof setTimeout>>();
  const show = useCallback((v: T) => {
    setValue(v); setVisible(true);
    if (timer.current) clearTimeout(timer.current);
    timer.current = setTimeout(() => setVisible(false), ms);
  }, [ms]);
  const hide = useCallback(() => { setVisible(false); if (timer.current) clearTimeout(timer.current); }, []);
...
export function useCooldown() {
  ...
  const start = useCallback((seconds: number) => {
    stop(); setTotal(seconds); setRemaining(seconds);
    timer.current = setInterval(() => setRemaining((r) => { if (r <= 1) { stop(); return 0; } return r - 1; }), 1000);
  }, [stop]);
  useEffect(() => stop, [stop]);
