...
|---|---|---|---|---|---|---|
| 1 | Authentication (register/login/refresh/logout/me/verify-email) | **PORT + KEEP+HARDEN — DONE (inc 1; label corrected inc 10 audit)** | `src/modules/auth/auth.routes.ts`, `auth.service.ts` (full flows, in-memory tested) | kernel auth routes + AuthService on the UserStore port; HTTP contract tests (authRoutes) + PGlite persistence tests (identityPersistence); register/login/refresh throttled inc 7 | C-inc1 | delivered on the Phase B foundation (no memory fallback, CSPRNG jti); matrix cell was stale (corrected inc 10 audit) |
...
| 4 | Users / profile model | **PORT — DONE (inc 1; label corrected inc 10 audit)** | `auth.service.ts` PublicUserDto (fullName, timezone, plan, aiConsent) | 0002 delivered full_name/timezone/plan/status/ai_consent_at + session metadata (locale pre-existed in 0001); profile TZ consumed by trades time interpretation (ADR-004) | C-inc1 | delivered; matrix cell was stale (corrected inc 10 audit) |
...
| 22 | API contracts (envelope/health/errors) | **PORT (PHP) — DONE (inc 1; label corrected inc 10 audit)** | errorHandler (close but ms `Z`) | 4-field C-10 envelope + `/health` per OD-3 + `/ready` split delivered (parity 6/6); error-object exact fields remain owner/fixture-gated (see §B) | C-inc1 | delivered; matrix cell was stale (corrected inc 10 audit) |
...
|---|---|---|
| Response envelope 4-field `{status, data, error, timestamp}` | **PRESERVE — DONE (inc 1)** | PHP `Response.php:36-46` (VERIFIED); Remote close but ms `Z` (fixture-pending detail) |
| `/health` → 200 `data:{status:'ok', time:<gmdate('c')>}` | **PRESERVE — DONE (inc 1, OD-3)** | PHP `api/index.php:43-45` (VERIFIED); OD-3 approved as reference; Local `checks.database` shape withdrawn; DB readiness moves to `/ready` (S8 liveness/readiness split) |
| Envelope timestamp format seconds `+00:00` | **PRESERVE** | PHP `gmdate('c')`; OD-5 |
| Auth routes `/api/v1/auth/*` | **PRESERVE — core set DONE (inc 1/2; email-dependent routes Phase I)** | Remote routes + PHP controller (VERIFIED); email-dependent routes deferred (Phase I) |
| Login/refresh → `{tokens:{accessToken, expiresIn, tokenType, user}}` + `refresh_token` cookie (Path=/, HttpOnly, Secure, SameSite=Lax, Max-Age=2592000) | **PRESERVE — DONE (inc 1)** | Remote `auth.routes.ts` + PHP `AuthController::respondWithTokens` (VERIFIED; PHP cookie TTL `jwt_refresh_ttl_sec` 2,592,000) |
| Auth error codes (INVALID_CREDENTIALS, EMAIL_ALREADY_REGISTERED, EMAIL_NOT_VERIFIED, INVALID_TOKEN, SESSION_EXPIRED, ACCOUNT_INACTIVE, VERIFICATION_LIMIT, VERIFICATION_RETRY_DELAY, REFRESH_COOKIE_MISSING, VALIDATION_FAILED) + statuses | **PRESERVE — DONE (inc 1)** | Remote `auth.service.ts`/`auth.routes.ts` (VERIFIED); PHP parity fixture-pending |
| Register → 201 `{verificationRequired, email}` (+`messageKey:'auth.verificationResent'` re-send case) | **PRESERVE — DONE (inc 1)** | Remote register (VERIFIED); PHP `$result` payload fixture-pending |
| verify-email → `{verified, alreadyVerified, messageKey, params}` | **PRESERVE — DONE (inc 1)** | Remote (VERIFIED); expired/unknown-token error code fixture-pending (INVALID_TOKEN chosen, documented) |
| Access token: HS256 JWT `{sub, role}`, TTL 900s | **PRESERVE — DONE (inc 1, hardened signer)** | Remote `jwt.ts` (ttl default 900 — VERIFIED); Local hardened signer (no fallback secret, CSPRNG jti — behavior-compatible, mechanism hardened) |
