...
| MG-G11 | Test suite covers migrated surface and passes | PARTIAL | PARTIAL | `CURRENT_RUNTIME_VERIFIED` — **AC-6 real-PG evidence exists**: 23 batteries, forward + reverse = 46 executions, 0 failures, 0 skips (anti-skip floors satisfied); migrations `0001…0022` all applied on the real server, second migration run applied 0 (idempotency); **PostgreSQL 17.11 was used (disposable local cluster) — PG16 remains the GHA-proven / compose target**; plus 804/804 local battery + `next build` exit 0 (AC-3/AC-5) | Coverage extends as capabilities land; D1 smoke expectation fix (MG-OBS-6, owner-gated) |
...
|---|---|---|---|---|---|---|
| MG-METAAPI-ASSEMBLY | MetaAPI sync creates **one trade per OUT fill** with `entry_price = exit_price`, volume = OUT fill volume, `contract_size` hardcoded 1, single timestamp; `ON CONFLICT DO NOTHING` never corrects. Legacy assembles per-position, volume-weighted, earliest-IN/latest-OUT. **Implementation brief prepared 2026-09-26:** `docs/reconciliation/METAAPI_POSITION_ASSEMBLY_BRIEF.md` (design only; owner decisions OD-M-PA-1…3 pending) | **P1** | OPEN | STATIC | §9.2; `apps/worker/src/metaapi/syncRepository.ts` importBatch; legacy `MetaApiDealAssembler::assemble()` + `MetaApiService::reconcileAccount`; brief | MG-G01, G04, G08; OD-M D-4; OD-M-PA-1…3 |
