...
| **QG-14** | GitHub Cost Policy          | `npm run github:cost:check`                                                           | Standard runners, timeouts ≤30m, no schedules (one narrow exception: the weekly Backup Retention maintenance workflow — see below), no `repository_dispatch`/`workflow_run`, no writes, retention ≤14d. | YES       | `SHARED_REQUIRED` |
...
### QG-14 scheduled-workflow exception (weekly retention maintenance)

Scheduled workflows remain **prohibited by default**. There is exactly one
exception, enforced in `scripts/validate-github-cost.ts`:

> The specific Backup Retention maintenance workflow may run **once every 7
> days** because it performs retention maintenance only and holds no
> deployment, migration, MetaAPI or worker authority.

The exception is granted only when **all three** conditions hold, and fails
closed otherwise:

1. **Identity** — path is exactly `.github/workflows/backup-retention.yml`
   (`SCHEDULED_WORKFLOW_ALLOWLIST`). A renamed copy is rejected.
2. **Frequency** — exactly one `cron`, matching `WEEKLY_CRON_RE` (fixed minute,
   fixed hour, single fixed day-of-week). Daily, hourly, `*/n` and
   multiple/twice-weekly schedules are rejected.
3. **Authority** — the workflow source, with comments stripped, must contain no
   deployment, migration, MetaAPI or worker capability.

Any other scheduled workflow still fails QG-14. This is deliberately **not** a
general "allow schedules" flag.

**Why weekly:** the retention _law_ is unchanged at **14 days** (336 hours,
1,209,600 seconds). The cron only controls how often the reaper looks for
backups that have _already_ become eligible. Weekly is ~52 scheduled runs/year
versus ~365 daily, i.e. **~85.8% fewer scheduled executions**, with no change to
eligibility.
